						...
						<div class="col-lg-12">
							<form enctype="multipart/form-data" action="{{ route('banner.store') }}" method="post">
								...
								<div id="filediv" class="col-12 col-md-3 col-sm-4 col-xs-12">
									...
									@endif
									<input name="bannertext" type="text" class="form-control bannertxt" id="textdt" placeholder="Enter Text">
					...
					<div class="row">
						<div class="col-lg-12 abcd position-absolute">							
							...
							<div class="col-12 col-md-3 col-sm-4 col-xs-12" id="dcs{{$k}}">
								...
								<button type="button" class="btn btn-sm delmainimg position-relative alert-danger" attrid="{{$banner->id}}" onclick="deletetbldocs('{{$banner->id}}','{{$k}}')">
								<i class="fa fa-trash-o"></i></button>
								<img src="{{ asset('public/uploads/banner/'.$banner->file_name) }}">
								<p style="border: 1px solid #c3bdbd;width:200px;float:right;clear:both;padding: 2px;margin-top:3px;">{{$banner->bannertext}}</p>
								<p>Uploaded On {{date('d M Y', strtotime($banner->created_at))}}</p>

						...
						<div class="col-lg-12">
							<form enctype="multipart/form-data" action="{{ route('activity.store') }}" method="post">
								...
								<div id="filediv" class="col-12 col-md-3 col-sm-4 col-xs-12">
									...
									@endif
									<input name="activitytext" type="text" class="form-control activitytxt" id="textdt" placeholder="Enter Text">
									@if ($errors->has('activitytext'))
									<span class="text-danger">
										<strong>{{ $errors->first('activitytext') }}</strong>
									</span>
									@endif
								</div>
								<div class="col-sm-12">
					...
					<div class="row">
						<div class="col-lg-12 abcd position-absolute">							
							...
							<div class="col-12 col-md-3 col-sm-4 col-xs-12" id="dcs{{$k}}">
								...
								<button type="button" class="btn btn-sm delmainimg position-relative alert-danger" attrid="{{$activity->id}}" onclick="deletetbldocs('{{$activity->id}}','{{$k}}')">
								<i class="fa fa-trash-o"></i></button>
								<img src="{{ asset('public/uploads/activity/'.$activity->file_name) }}">
								<p style="border: 1px solid #c3bdbd;width:200px;float:right;clear:both;padding: 2px;margin-top:3px;">{{$activity->activitytext}}</p>
								<p>Uploaded On {{date('d M Y', strtotime($activity->created_at))}}</p>
